...
// Use Node Sass https://sass-lang.com/dart-sass.
...
gulp.task('css-rtl', () => {
  return gulp
    .src(`${gulpConfig.css.to}/**/*.css`)
    .pipe($.rtlcss())
    .pipe(
      $.rename((p) => {
        if (p.basename.endsWith('.min')) {
          p.basename = p.basename.replace('.min', '-rtl.min');
        } else {
          p.basename += '-rtl';
        }
...
gulp.task('js', () => {
  return gulp
    ...
    .pipe(
      $.if(
        (file) => process.env.NODE_ENV !== 'production' && !file.path.match(/-init.js$/),
        gulp.dest(gulpConfig.js.to)
      )
    )
    .pipe(
      $.if((file) => process.env.NODE_ENV !== 'dev' && !file.path.match(/-init.js$/), $.uglify())
    )
    .pipe(
      $.if(
...
gulp.task('zip', () => {
  ...
  return gulp
    .src([`${fromPath}/**/*`, `${fromPath}/**/.*`, `!${gulpConfig.deploy.zip.to}`], {
      base: fromPath,
      follow: true,
    })
 ...
 */
gulp.task('build', (cb) => {
  process.env.NODE_ENV = process.env.NODE_ENV || 'build';
  gulp.series('clean', 'images', 'html', 'css', 'css-rtl', 'js', 'static')(cb);
});
...
gulp.task('default', (cb) => {
  ...
  gulp.series('clean', 'images', 'html', 'css', 'css-rtl', 'js', 'static', 'watch')(cb);
});
